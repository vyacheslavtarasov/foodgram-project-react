...
def api_subscriptions(request):
    ...
    for entry in my_subscriptions:
        ...
        if recipes_limit is not None and my_recipes.count() > int(recipes_limit):
            continue
        data2return = {}
...
def api_subscribe(request, id):
    ...
    if request.method == 'POST':
        ...
        if recipes_limit is not None and my_recipes.count() > int(recipes_limit):
            return Response({"error": "amount of recepies is too big"}, status=400)
        ...
        Subscribe.objects.filter(user_subscribed_on=my_user_subscribe_on, user=request.user).delete()
        return Response(status=204)
